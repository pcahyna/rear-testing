...
rlJournalStart
    rlLog "NFS_SERVER_IP = $NFS_SERVER_IP"
...
GRUB_RESCUE=y' | tee $REAR_CONFIG" 0 "Creating basic configuration file"
        ...
        rlPhaseStartSetup
            rlLog "Setup GRUB"
            rlRun "sed -r -i 's/linux \/rear-kernel\\s+selinux=0\\s+console=ttyS0,9600/& console=tty0 unattended/' /boot/grub2/grub.cfg"
            rlRun "echo \"set saved_entry='Relax-and-Recover'\" >> /boot/grub2/grub.cfg"
            rlRun "echo 'set default=\${saved_entry}' >> /boot/grub2/grub.cfg"
        rlPhaseEnd
